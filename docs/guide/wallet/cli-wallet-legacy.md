...
Please refer to the [Abelian Applications Basics](/guide/) to understand the software architecture first. The legacy wallet (`abewalletlegacy`) works with the legacy Abelian address format and is primarily maintained for existing users.

Visit the [Official Abelian Download Page](/downloads/latest#abelian-cli-wallet-legacy) to download the software.
...
> We strongly recommend migrating to the [Multi-Layer Privacy Wallet CLI](/guide/wallet/cli-wallet-mlp) for enhanced
...
Instead, you can create a file called `arg1` in the configuration folder of `abec` and add the receiver's address and amount into it. The format is as follows:
...
4. **Backup the abewalletlegacy configuration folder.** Please copy the folder to somewhere else which is also safe. This, together with the public and private passphrase, may help rollback to the current wallet in case your mnemonics are not as you expected.
...
the [Wallet Migration Guide](/guide/wallet/cli-wallet-mlp#_7-migration-from-legacy-wallet) in the MLP wallet documentation.
:::
