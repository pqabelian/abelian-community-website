...
**MLP（多層隱私）地址** 是一種升級後的格式，旨在增強隱私性和功能性。它支援多種隱私層級，例如全隱私模式、偽隱私模式，以及偽隱私機密交易（Confidential Transaction）模式，縮寫為 CT，偽隱私的升級版，屬於 Transparent 的一種。
...
- **網路前綴**：用於識別地址類型（例如 Legacy（經典）、全隱私、偽隱私、偽隱私-CT）。
  - **Legacy 短地址（主網）**：`abe1`
  - **Legacy 短地址（測試網）**：`abe3`
  - **MLP 短地址（主網）**：`abe010`
  - **MLP 短地址（測試網）**：`abe012`
...
- **增強隱私性**：MLP 格式引入了不同的隱私層級，允許用戶根據需求選擇全隱私、偽隱私或偽隱私-CT 選項。
